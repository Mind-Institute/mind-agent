...
begin
  ...
  insert into engagement.identidades (mind_id, canal, identificador, verificado, confianca) values
    (a, 'whatsapp', tel_a, true, 'alta'),
    (a, 'hubspot',  hub_a, true, 'alta');
  insert into crm.contato_espelho (hubspot_id, email, phone, firstname, lastname, mind_id)
  ...
  select proposta into prop from engagement.identidade_fusoes
   where status = 'pendente' and participante_origem is not null
     and least(mind_id, participante_origem) = least(a, b)
     and greatest(mind_id, participante_origem) = greatest(a, b)
   limit 1;
  ...
  if (select mind_id from engagement.identidades where canal = 'email' and identificador = mail_e) <> b then
  ...
  r := public.mind_fusao_decidir((select id::text from engagement.identidade_fusoes where status = 'pendente' and participante_origem is not null
                                    and least(mind_id, participante_origem) = least(a, b) and greatest(mind_id, participante_origem) = greatest(a, b) limit 1),
                                 'aprovar', 'contrato_d5');
  ...
  if (select mind_id from crm.contato_espelho where hubspot_id = hub_a) <> b then raise exception 'D5.3 contato_espelho não foi repontado'; end if;
  ...
  if exists (select 1 from engagement.identidade_fusoes where status = 'pendente' and participante_origem is not null
              and least(mind_id, participante_origem) = least(a, b)) then
  ...
  insert into crm.leads_capturados (firstname, lastname, email) values ('Cedo', 'Demais', mail_cedo) returning mind_id, mind_id_criterio into f, crit;
  ...
  if (r->>'pessoas_criadas')::int < 1 or (select mind_id from crm.leads_capturados where email = mail_cedo) is null then
  ...
  if (select mind_id from engagement.identidades where canal = 'cpf' and identificador = '52998224725') is null then
  ...
  select mind_id into d from eduzz.ingressos where uuid = 'd5-ingresso-' || suf;
  if d is null or d = c then raise exception 'D5.4 CPF igual com e-mail diferente deveria ser pessoa diferente (d=%, c=%)', d, c; end if;
  if exists (select 1 from engagement.identidade_fusoes where status = 'pendente' and participante_origem is not null
              and least(mind_id, participante_origem) = least(c, d) and greatest(mind_id, participante_origem) = greatest(c, d)) then
    raise exception 'D5.4 CPF sozinho abriu proposta de fusão';
  end if;
  if (select mind_id_criterio from eduzz.ingressos where uuid = 'd5-ingresso-' || suf) not like 'criada:%email%' then
    raise exception 'D5.4 critério esperado "criada: …email…", veio %', (select mind_id_criterio from eduzz.ingressos where uuid = 'd5-ingresso-' || suf);
  end if;
  ...
  -- ---------- 5. o trigger carimba mind_id na escrita, e não repete a porta à toa ----------
  insert into credenciamento_summit_2026.participantes (id, name, email, cellphone, status, sincronizado_em)
    ...
    returning mind_id into e;
  if e <> b then raise exception 'D5.5 participante deveria cair em B pelo e-mail/WhatsApp, caiu em %', e; end if;
  if (select mind_id_criterio from credenciamento_summit_2026.participantes where lower(email) = mail_e) !~ 'email' then
    raise exception 'D5.5 critério deveria citar email';
  end if;
  if not exists (select 1 from engagement.identidades where mind_id = b and canal = 'credenciamento') then
  ...
  insert into crm.leads_capturados (firstname, lastname) values ('Sem', 'Identificador') returning mind_id, mind_id_criterio into f, crit;
  ...
  select count(*) into n from engagement.identidades where mind_id = b;
  r := public.mind_pessoa_enriquecer(b);
  select count(*) into n2 from engagement.identidades where mind_id = b;
  ...
  insert into engagement.identidades (mind_id, canal, identificador, verificado, confianca) values (f, 'email', mail_ana, false, 'media');
  insert into pessoas.pessoas (whatsapp, primeiro_nome, origem) values (tel_g, 'Beatriz', 'bot') returning id into g;
  insert into engagement.identidades (mind_id, canal, identificador, verificado, confianca) values (g, 'whatsapp', tel_g, true, 'alta');
  ...
  select proposta into prop from engagement.identidade_fusoes where status = 'pendente' and participante_origem is not null
    and least(mind_id, participante_origem) = least(f, g) and greatest(mind_id, participante_origem) = greatest(f, g) limit 1;
  ...
  if exists (select 1 from engagement.identidades where mind_id = k and canal = 'whatsapp') then raise exception 'D5.9 o telefone da compradora foi colado ao colega'; end if;
  ...
  if exists (select 1 from engagement.identidade_fusoes where status = 'pendente' and participante_origem is not null
              and least(mind_id, participante_origem) = least(s, k) and greatest(mind_id, participante_origem) = greatest(s, k)) then
  ...
  insert into eduzz.vendas (linha_origem, fatura, cliente_nome, cliente_email, cliente_fones, cliente_documento, sincronizado_em)
    ...
    returning mind_id, mind_id_criterio into k2, crit;
  ...
  if exists (select 1 from engagement.identidade_fusoes where status = 'pendente' and participante_origem is not null
              and least(mind_id, participante_origem) = least(s, k2) and greatest(mind_id, participante_origem) = greatest(s, k2)) then
  ...
  insert into eduzz.vendas (linha_origem, fatura, cliente_nome, cliente_email, cliente_fones, sincronizado_em)
    ...
    returning mind_id, mind_id_criterio into s2, crit;
  ...
  select proposta into prop from engagement.identidade_fusoes where status = 'pendente' and participante_origem is not null
    and least(mind_id, participante_origem) = least(s, s2) and greatest(mind_id, participante_origem) = greatest(s, s2) limit 1;
  ...
  insert into eduzz.ingressos (uuid, participante, email, email_comprador, cod_participante, sincronizado_em)
    ...
    returning mind_id, mind_id_criterio into e, crit;
  ...
  insert into eduzz.ingressos (uuid, participante, email, email_comprador, cod_participante, sincronizado_em)
    ...
    returning mind_id, mind_id_criterio into r2, crit;
  ...
  if (select mind_id from engagement.identidades where canal = 'email' and identificador = mail_s) <> s then raise exception 'D5.12 o e-mail mudou de dona'; end if;
  select proposta into prop from engagement.identidade_fusoes where status = 'pendente' and participante_origem is not null
    and least(mind_id, participante_origem) = least(s, r2) and greatest(mind_id, participante_origem) = greatest(s, r2) limit 1;
  ...
  insert into crm.leads_capturados (firstname, lastname, email) values ('Roberta', 'Diferente', mail_s) returning mind_id, mind_id_criterio into f, crit;
  ...
  if exists (select 1 from engagement.identidades where mind_id = s and identificador in (mail_k, mail_k2, mail_s2)) then
  ...
  if (select mind_id from crm.contato_espelho where hubspot_id = hub_z) is not null then
  ...
  if exists (select 1 from engagement.identidades where mind_id = s and identificador in (mail_z, hub_z)) then raise exception 'D5.13 identificadores do Zeca colados à Sueli'; end if;

  -- ---------- 8. a pessoa com todos os ids, numa linha ----------
  if not exists (select 1 from pessoas.v_pessoa_360 v where v.mind_id = b
