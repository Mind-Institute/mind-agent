alter table agentes.kit_blocos
  drop constraint if exists kit_blocos_secao_valida;

alter table agentes.kit_blocos
  add constraint kit_blocos_secao_valida
  check (secao = any (array['structured'::text,'knowledge'::text,'tools'::text,'decisioning'::text]));

insert into agentes.kit_blocos
  (rota, bloco, provider, secao, obrigatorio, ativo)
values
  ('concierge_summit','product_decisioning','agentes.prompts','decisioning',false,true)
