-- mindagent_chat_search: pergunta ampla volta a listar quando o assunto nao acha nada.
--
-- O BUG, medido em producao: quem escreve COM acento recebia zero.
--
--   "quais palestrantes estarao no summit?"    -> 8 palestrantes
--   "quais palestrantes estarao no Summit?"    -> 0   (com acento em "estarao")
--   "quais palestrantes vao estar no evento?"  -> 0   (com acento em "vao")
--   "speakers"                                 -> 0
--
-- A causa e o guarda `n_foco = 0` das duas listagens. O "andaime da pergunta" -- as
-- palavras que estruturam a frase em vez de dizer o assunto dela -- foi escrito nas
-- formas SEM acento (`estara`, `vao`), mas o stemmer portugues devolve `estara` e `va`
-- de quem escreve certo. Sobrando um lexema, `n_foco` vira 1, a listagem e desligada e
-- o bloco volta vazio. O agente entao diz com honestidade que nao tem a lista, porque
-- de fato nao recebeu nada.
--
-- Enumerar conjugacao na lista consertaria esta frase e quebraria na proxima. A
-- correcao e estrutural: quando a pergunta PEDE a lista e a busca por assunto nao
-- encontrou nada, entrega a lista. O filtro estreito continua intacto -- e esse e o
-- ponto: "palestrantes sobre lideranca" devolve 1, nao 8, porque ali o assunto ACHOU e
-- nao ha queda.
--
-- POR QUE O CORPO INTEIRO, E NAO UM PATCH SOBRE A DEFINICAO VIVA:
-- durante o diagnostico producao recebeu um hotfix direto na lista de `foco_lex`
-- (`vao`, `estar`, `estara`, e `esta` duplicado) que nunca entrou na cadeia de
-- migrations. Um patch textual sobre `pg_get_functiondef()` preservaria esse texto em
-- producao e nao o teria num banco reconstruido pela cadeia: dois corpos diferentes
-- com a mesma versao. Esta migration reescreve a funcao a partir do corpo canonico de
-- `20260830223000` com as quatro mudancas acima, entao os dois estados convergem para
-- a MESMA definicao. O hotfix nao vira regra: a queda estrutural o substitui.
