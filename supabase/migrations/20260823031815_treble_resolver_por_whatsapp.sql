-- Corrige o nome (e WhatsApp, nao telefone) e o append do array.
drop function if exists public.treble_resolver_por_telefone(text);

create or replace function public.treble_resolver_por_whatsapp(p_whatsapp text)
returns uuid
language plpgsql stable security definer
set search_path to 'public','crm','engagement'
as $function$
declare
  v_digitos text := regexp_replace(coalesce(p_whatsapp,''), '\D', '', 'g');
  v_pessoa  uuid;
begin
...
revoke all on function public.treble_resolver_por_whatsapp(text) from public, anon, authenticated;
grant execute on function public.treble_resolver_por_whatsapp(text) to service_role;
...
begin
  ...
  -- PRIMEIRA BUSCA: pela chave que veio (o WhatsApp).
...
comment on function public.treble_agent_start(text,jsonb,text,text) is
  'Abre/retoma a conversa do Treble e faz a PRIMEIRA busca da pessoa pelo WhatsApp. Nao cria pessoa: devolve falta[] para o agente pedir so o que falta. A criacao acontece em treble_agent_identificar, depois da segunda busca.';