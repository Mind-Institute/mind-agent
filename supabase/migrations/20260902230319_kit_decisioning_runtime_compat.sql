...
begin
  ...
  select string_agg(pr.conteudo, E'\n\n' order by k.bloco)
    ...
    join agentes.prompts pr
      on pr.chave = k.bloco and pr.ativo
   where k.rota = v_rota
     and k.ativo
     and k.secao = 'decisioning';

  -- Compatibilidade com os runtimes atuais: `playbook` é o bundle de instruções
  -- consumido pela Edge. As fontes continuam separadas no registry/prompt e o
  -- Decisioning também sai em campo próprio para auditoria e futuros consumidores.
  if nullif(btrim(coalesce(v_decisioning, '')), '') is not null then
    v_playbook := concat_ws(E'\n\n', nullif(btrim(coalesce(v_playbook, '')), ''), v_decisioning);
  end if;
