-- E-mail NAO identifica pessoa nesta base: quem compra tres ingressos usa o
-- proprio e-mail e CPF e depois atribui para tres pessoas diferentes. O e-mail
-- de um ingresso e de quem COMPROU, nao de quem vai.
--
-- Entao: e-mail que bate e sinal forte, nao prova. Se o nome informado for
-- diferente do nome de quem ja tem aquele e-mail, nao liga e nao cria --
-- marca conflito e devolve o nome de quem esta la, para o agente perguntar.

drop function if exists public.treble_agent_identificar(text,text,text,text);
...
begin
  ...
  -- SEGUNDA BUSCA. O WhatsApp e chave forte (e do aparelho de quem fala).
  -- O e-mail e chave fraca: pode ser o de quem comprou para outra pessoa.
  ...
  -- O nome bate com quem ja tem esse e-mail? Se nao bate, e outra pessoa.
  if v_por_email is not null and v_pessoa is null and not p_confirmar_mesmo_email then
    if v_nome is not null and v_dono_nome is not null
       and lower(unaccent(v_nome)) <> lower(unaccent(v_dono_nome)) then
  ...
  if v_pessoa is null then
    ...
    -- e-mail ja usado por outra pessoa: a nova entra sem e-mail proprio,
    -- identificada pelo WhatsApp. O e-mail fica so como identidade de canal.
...
comment on function public.treble_agent_identificar(text,text,text,text,boolean) is
  'SEGUNDA busca da pessoa. WhatsApp e chave forte; e-mail e chave FRACA porque quem compra varios ingressos usa o proprio e-mail para outras pessoas. Se o e-mail bate mas o nome nao, devolve conflito e nao liga nem cria ate o agente confirmar.';