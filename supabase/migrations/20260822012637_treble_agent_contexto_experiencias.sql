...
-- passam a estar SEMPRE no contexto do agente — são o coração da venda
-- consultiva, não podem depender da busca casar com a frase do lead.
...
select jsonb_build_object(
  ...
  'experiencias_o_que_inclui', (
    ...
    from mind.knowledge_documents k where k.tipo_conteudo = 'ingresso'),
  'faq', (
    select coalesce(jsonb_agg(jsonb_build_object('titulo', k.titulo, 'texto', left(k.corpo, 1200))), '[]'::jsonb)
    from mind.knowledge_documents k where k.tipo_conteudo = 'faq'),
