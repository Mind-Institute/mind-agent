-- LGPD: a pergunta de identidade NUNCA revela dado de terceiro.
-- "Achei um cadastro no nome de Mariana Costa" entrega o nome do dono do
-- e-mail para quem quer que esteja do outro lado -- e permite enumerar a base
-- digitando e-mails. A config do proprio Mind ja diz so_dados_do_titular=true.
--
-- Entao a pergunta so pede o que a pessoa ja sabe sobre si mesma. O sistema
-- compara em silencio. Consequencia aceita: quem se apresenta por apelido vira
-- cadastro novo. Duplicata se conserta depois; vazamento nao.

create or replace function public.treble_pergunta_de_identidade(p_candidatos jsonb)
returns jsonb
language sql stable
as $function$
  with c as (select value p from jsonb_array_elements(coalesce(p_candidatos,'[]'::jsonb))),
  n as (select count(*) q from c)
  select case
    when (select q from n) = 0 then null

    -- Um candidato, nome destoante: pede o nome completo DELA. Nao diz o que temos.
    when (select q from n) = 1 and (select (p->>'precisa_confirmar_nome')::boolean from c) then
      jsonb_build_object(
        'tipo','confirmar_nome_do_titular',
        'pergunta','So para eu registrar certinho: qual o seu nome completo?',
        'revela_terceiro', false)

    when (select q from n) = 1 then null

    -- Varios candidatos: pergunta sobre ELA, no campo que difere entre os
    -- candidatos. O valor que difere nunca aparece na pergunta.
    else coalesce(
      (select jsonb_build_object('tipo','desempate','campo',campo,'pergunta',pergunta,
                                 'revela_terceiro', false)
         from (
           select 'comprou' campo, count(distinct p->>'comprou') d,
                  'So confirmando: voce ja comprou ingresso para o Summit?' pergunta from c
           union all
           select 'empresa', count(distinct p->>'empresa'),
                  'De qual empresa voce e?' from c
           union all
           select 'cargo', count(distinct p->>'cargo'),
                  'Qual e o seu cargo?' from c
           union all
           select 'nome', count(distinct p->>'nome'),
                  'Me confirma seu nome completo?' from c
         ) x where d > 1
         order by case campo when 'comprou' then 1 when 'empresa' then 2
                             when 'cargo' then 3 else 4 end limit 1),
      jsonb_build_object('tipo','desempate','campo',null,
        'pergunta','Me confirma seu nome completo?','revela_terceiro', false))
  end;
$function$;

revoke all on function public.treble_pergunta_de_identidade(jsonb) from public, anon, authenticated;
grant execute on function public.treble_pergunta_de_identidade(jsonb) to service_role;

comment on function public.treble_pergunta_de_identidade(jsonb) is
  'Pergunta que desfaz duvida de identidade SEM revelar dado de terceiro (LGPD, so_dados_do_titular). Nunca cita nome, e-mail ou empresa de quem ja esta na base: pergunta sobre a propria pessoa e compara em silencio.';