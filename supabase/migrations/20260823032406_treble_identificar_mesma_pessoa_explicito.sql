-- Corrige: "confirmar" era ambiguo e ligava o convidado no cadastro do comprador.
-- Agora a resposta e explicita:
--   p_mesma_pessoa = null  -> ainda nao perguntou: devolve conflito
--   p_mesma_pessoa = true  -> "sim, sou eu": liga na pessoa do e-mail
--   p_mesma_pessoa = false -> "nao, so uso o e-mail dela": cria pessoa nova,
--                             sem tomar o e-mail de quem ja o tem.
...
declare
  ...
  v_por_email uuid;
  v_dono_nome text;
  v_nomes_diferentes boolean := false;
...
begin
  ...
  -- WhatsApp e chave forte (o aparelho de quem fala).
  -- E-mail e chave FRACA: quem compra varios ingressos usa o proprio e-mail
  -- para as outras pessoas. Os dois viram contato no CRM, mas so depois da
  -- atribuicao -- que pode demorar meses. Antes disso so o comprador existe.
  ...
  v_nomes_diferentes := v_por_email is not null and v_nome is not null and v_dono_nome is not null
    and public.mind_nome_simples(v_nome) is distinct from public.mind_nome_simples(v_dono_nome);

  -- Nome diferente e ninguem perguntou ainda: para e devolve o conflito.
  if v_pessoa is null and v_nomes_diferentes and p_mesma_pessoa is null then
  ...
  if v_pessoa is null then
    if v_nomes_diferentes and p_mesma_pessoa is false then
      v_pessoa := null;          -- e outra pessoa: cria, sem tomar o e-mail
    else
      v_pessoa := v_por_email;   -- e a mesma pessoa (ou o nome batia)
    end if;
  ...
  -- o e-mail so vira identidade de quem realmente e o titular
  ...
  return (
    select jsonb_build_object(
      ...
      'usa_email_de_terceiro', (v_nomes_diferentes and coalesce(p_mesma_pessoa,true) is false),
