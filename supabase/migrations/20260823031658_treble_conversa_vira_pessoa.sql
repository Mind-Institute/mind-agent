-- Passo 1: a conversa do Treble passa a encontrar a pessoa.
-- Busca pela chave que veio (telefone). Acha -> liga. Nao acha -> NAO cria:
-- devolve o que falta, para o agente pedir e a segunda busca acontecer
-- em treble_agent_identificar. E a segunda busca que impede a duplicata.

create or replace function public.treble_resolver_por_telefone(p_telefone text)
returns uuid
...
declare
  ...
  v_pessoa  uuid;
begin
  if length(v_digitos) < 10 then return null; end if;

  -- 1. a chave canonica de canal
  select i.pessoa_id into v_pessoa
  from engagement.identidades i
  where i.canal = 'whatsapp' and i.identificador = v_digitos
  limit 1;
  if v_pessoa is not null then return v_pessoa; end if;

  -- 2. fallback: quem ja esta no CRM com esse whatsapp
  select p.id into v_pessoa
  from crm.pessoas p
  where regexp_replace(coalesce(p.whatsapp,''), '\D', '', 'g') = v_digitos
  limit 1;
...
begin
  ...
  -- PRIMEIRA BUSCA: pela chave que veio.
  v_pessoa := conv.participante_id;
  v_digitos := regexp_replace(coalesce(conv.telefone,''), '\D', '', 'g');

  if v_pessoa is null then
    v_pessoa := public.treble_resolver_por_telefone(conv.telefone);
  end if;

  if v_pessoa is not null then
    -- garante a identidade de canal e cola a conversa na pessoa
    ...
    select jsonb_strip_nulls(jsonb_build_object(
             'pessoa_id', p.id, 'primeiro_nome', p.primeiro_nome,
...
revoke all on function public.treble_resolver_por_telefone(text) from public, anon, authenticated;
grant execute on function public.treble_resolver_por_telefone(text) to service_role;

comment on function public.treble_agent_start(text,jsonb,text,text) is
  'Abre/retoma a conversa do Treble e faz a PRIMEIRA busca da pessoa pela chave que veio. Nao cria pessoa: devolve falta[] para o agente pedir o que falta, e a criacao so acontece em treble_agent_identificar, depois da segunda busca.';