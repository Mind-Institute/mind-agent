...
| `mindagent-acesso` | o primeiro login com o Google da Mind — `POST /admin/vincular` liga a conta Google à pessoa da lista de admins do sistema pela porta `mind_admin_vincular_login` (migration `20260925232508`): conta Google, e-mail verificado @joinmind.com.br, uma pessoa só com esse e-mail, acesso ativo e marca de equipe. Função nova, `verify_jwt = false` porque valida a sessão por dentro e o preflight de CORS não leva token. Publicada em 25/09/2026. Desde 26/09 também a tela de admins do sistema: `GET /admin/admins` (busca, `ativo`, `papel`), `GET /admin/admins/:id`, `POST /admin/admins` `{ email, papel }` e `PATCH /admin/admins/:id` `{ papel?, ativo? }` com `If-Unmodified-Since-Version`, pelas portas `mind_admin_read_admins` e `mind_admin_mutate_admins` (migrations `20260926144621` e `20260926144815`, só `service_role`). **Versão 3 viva = este código** (26/09). Contratos do banco: `tests/admins_do_sistema_contract.sql` → `ADMINS_OK` e `tests/admins_no_painel_contract.sql` → `ADMINS_PAINEL_OK` (rodado em produção, sem rastro); a porta: `tests/mindagent_acesso_comportamento.test.mjs` |
| `mindagent-catalogo` | o Catálogo do painel — lê e edita `catalogo.produtos` pelas portas `mind_admin_read_catalogo` / `mind_admin_mutate_catalogo` (migration `20260925183716`), com a mesma autorização da `mindagent-home` (`mind_admin_users`). Função nova, `verify_jwt = false` porque valida a sessão por dentro e o preflight de CORS não leva token. Publicada em 25/09/2026. Desde 26/09 (`1.1.0`) ordena por qualquer coluna da tela (`?ordenar=campo` ou `-campo`, vazio no fim), a lista inteira antes de paginar — `tests/mindagent_catalogo_ordem.test.mjs`; `1.2.0` traduz a recusa `datas_da_turma` (no Institute as datas vêm de `institute.programas`, migration `20260926153820`). **Versão 5 viva = este código** (26/09). Contrato do banco: `tests/catalogo_painel_contract.sql` → `CATALOGO_OK` |
| `mindagent-summit` | as tabelas do Summit no painel, **só leitura** — hoje a programação do Mind Summit 2026: `GET /admin/summit_2026_sessions` (busca, `dia`, `tipo`, `espaco_id`, `precisa_reserva`, ordem por coluna, paginação) e `/:id`, pela porta `mind_admin_read_summit_2026_sessoes` (migration `20260926154547`, só `service_role`). Qualquer papel ativo de `mind_admin_users` lê. Função nova (26/09/2026), `verify_jwt = false` porque valida a sessão por dentro. **Versão 1 viva = este código**; conferida via `pg_net`: `health` 200, sem login 401, origem estranha 403, token falso 401. Contrato do banco `tests/summit_programacao_contract.sql` → `SUMMIT_PROGRAMACAO_OK`; a porta: `tests/mindagent_summit_comportamento.test.mjs` |
