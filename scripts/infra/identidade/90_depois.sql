...
select jsonb_pretty(jsonb_build_object(
  ...
  'motivos_sem_pessoa_hubspot', (select coalesce(jsonb_object_agg(pessoa_criterio, n), '{}'::jsonb) from (select pessoa_criterio, count(*) n from crm.contato_espelho where pessoa_id is null group by 1) s),
  -- verificações: têm de ser zero
  'identidade_apontando_para_pessoa_fundida', (select count(*) from engagement.identidades i join pessoas.pessoas p on p.id = i.pessoa_id where p.fundida_em is not null),
  ...
  'linhas_de_fonte_apontando_para_pessoa_fundida', (select count(*) from crm.contato_espelho c join pessoas.pessoas p on p.id = c.pessoa_id where p.fundida_em is not null)
    + (select count(*) from credenciamento_summit_2026.participantes c join pessoas.pessoas p on p.id = c.pessoa_id where p.fundida_em is not null),
