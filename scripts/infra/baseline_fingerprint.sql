...
-- ============================================================================
-- O que é:
--   Estende as 7 classes de `archive/pre-architecture/structural-baseline/reproduce.sql`
--   para 14 classes + contagens + roles exigidos. Serve para provar fidelidade
--   do baseline gerado por `scripts/infra/baseline_generator.sql`.
--
-- Como usar:
--   1. Rodar em PRODUÇÃO agora  -> guardar o JSON como referência congelada.
--   2. Rodar na PREVIEW depois do baseline aplicado -> comparar classe a classe.
--   Devolve 1 linha / 1 coluna `fingerprint` (jsonb).
--
-- Leitura do resultado:
--   - schemas/extensions/tables/columns/constraints/indexes/sequences/views/
--     functions/triggers/policies/grants/comments  -> devem BATER (md5 igual).
--     Qualquer diferença é infidelidade do gerador, salvo o delta explicado
--     pelos objetos do 12B se a preview já tiver rodado 12B.1A/1B.
--   - default_acls -> devem bater por BOOTSTRAP da branch, não por emissão.
--     O baseline não emite ALTER DEFAULT PRIVILEGES. Se diferir, é achado.
--   - required_roles -> todos precisam existir na branch, senão o baseline
--     aborta ao aplicar GRANT/CREATE POLICY.
--
-- Segurança: apenas SELECT sobre catálogos do sistema. Não lê dado de negócio,
-- não escreve, não altera nada. Escopo de schemas = filtro canônico do
-- reproduce.sql (exclui só schemas gerenciados do Supabase); objetos
-- pertencentes a extensões são excluídos via pg_depend.deptype = 'e'.
-- ============================================================================
...
-- ── classes ────────────────────────────────────────────────────────────────
f_schemas as (
  select nspname as v from scope
),
f_ext as (
...
f_policies as (
  select t.nspname || '.' || t.relname || '|' || p.polname
         ...
         || '|cmd=' || p.polcmd::text
         || '|roles=' || coalesce((
              select string_agg(pg_get_userbyid(u.r), ',' order by pg_get_userbyid(u.r))
...
f_defacl as (
  -- NÃO entra no baseline (é bootstrap do projeto Supabase, roles de plataforma).
  -- Entra aqui só para PROVAR que a branch nasce com os mesmos.
  select coalesce(pg_get_userbyid(d.defaclrole), '?') || '|'
         || coalesce(n.nspname, '<all>') || '|'
...
f_roles as (
  -- roles que a branch PRECISA ter para o baseline aplicar sem abortar
  select distinct pg_get_userbyid(u.r) as v
  from pg_policy p
  join tbl t on t.oid = p.polrelid
...
select jsonb_build_object(
  ...
  'tables_by_schema', (select jsonb_object_agg(nspname, n)
                       from (select nspname, count(*) as n from tbl group by nspname) q)
) as fingerprint;
