-- Os dois consumidores do banco comum passam a respeitar a janela de vigência.

-- 1) Concierge: filtro de vigência no bloco "mind".
...
-- 2) Treble (WhatsApp): mesmos filtros de vigência nos três blocos que leem o banco comum.
CREATE OR REPLACE FUNCTION public.treble_agent_context_base()
 RETURNS jsonb
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public', 'mind', 'treble'
AS $function$
select jsonb_build_object(
