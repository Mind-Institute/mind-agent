-- O agente precisa saber que dia e hoje EM RELACAO AO PRODUTO (Adriana):
-- antes do evento e venda; depois do evento, a mesma pergunta e atendimento.
-- Sem isso o bot tenta vender um Summit que ja aconteceu.
--
-- A conta sai daqui, nao do LLM: fase, dias que faltam e o que fazer em
-- cada fase. Se a linha tiver uma proxima edicao cadastrada, ela vem junto
-- para o agente redirecionar em vez de dizer "acabou".
...
comment on function public.mind_calendario(text) is
  'Onde estamos no calendario DESTE produto: fase, dias que faltam e o que o agente deve fazer. Depois do evento, venda vira atendimento.';

-- Entra no contexto oficial, ao lado do momento (hora do dia).
